...
and the expectations placed on smaller firms, must be read. %LI: weird sentence - rewrite! The cohort is not
...
2020)%LI: random source, maybe better this link: https://www.mdpi.com/2071-1050/12/24/10393?utm_source=chatgpt.com - look for real source!. The most advanced reporters in a sector therefore mark its practical
...
uncovered, completeness is the wrong yardstick for a SME, and the question
...
operation (Profand), and processing/branded firms (Nomad Foods, Espersen) %LI: still weird frasing because we only focus on processing part of company - so it's rather as effect of processing firms sourcing different fish species and origin?.
...
%LI: should we include turnover and ranking of eu market? + Maybe add that we chose so we have geographical coverage from south to north and added thai union as one of biggest processors in eu? 
%LI: this doesn't source of all of them but quite a few: https://www.mordorintelligence.com/industry-reports/europe-seafood-market/companies
%LI: should we say we also chose companies that we have contact with so back and forths are possible? or not professional?
\begin{table}[ht]
\centering
...
\end{tabular}
\end{table}
%LI: ensure table is aligned with text! 
...
year~2024 and, where already published, 2025 (the explorative year-over-year comparison later
...
cohort firms for informal review (a member-checking step;
...
%LI: maybe fig 5 should be mentioned earlier as this creates the frontier - meaning you have in red dotted what is max to cover and then what everyone actually covers - could we draw in notebook the frontier of topics on this like said in text?
...
is deepest on own-workforce disclosure (S1, 16 of 17) and affected communities (S3, 5 of 5), Espersen is thin on
S1 (4 of 17) and S3 (0 of 5) but complete on consumers/end-users (S4, 5 of 5), and Mowi
...
%LI: do we need an explanation of what each E1 - G1 means? or we think they will know?
...
smallest (Figure~\ref{fig:p1-esg}). %LI: shouldn't there be a nuance of fact that in general there are also more Social DR to report upon? if you look at split of DR (70 in total) across E, S, G is also not equal so shouldn't we do analysis of percentually how much is covered from DRs to be on equal footing (check notebook whether this is done!)
...
consumer disclosure. (%LI: is there a graph or something else for this - if not build in notebook, because cannot be unsourced!)
...
across all four possible sourcing models - wild-capture, aquaculture, integrated and
processing/brand alike %LI: it has to be clear that we're still talking about processing companies, but that they just have different integration models!
Business model changes only the \emph{edges} of the
...
while the climate, water, governance and workforce core is common to all (%LI: again is there a graph for this, if not we need to create one!)
...
cliff edge %LI: what does this mean/ show? Second, \emph{cohort-size invariance}: re-deriving the near-universal
...
cohort. %LI: check notebook - is this actually true - do we have a figure? 
Thai Union itself, the single non-EU firm and the broadest reporter (53 of
...
faces (%LI: source) - yet only one requirement is reported uniquely by it, so the consensus
...
\label{fig:p1-year}
\end{figure}
%LI: we should in notebook ensure that the DRs added and dropped are named! otherwise fig 6 - b not very indicative!
...
\& Ebner, 2010); the contribution here is to make that step
