...
European seafood processors actually disclose?} It establishes that practice from primary documents, setting the assumed \textit{upper bound} of sector disclosure depth. Both the demand mapping that follows and the expectations placed on smaller firms should be read against it. The cohort is not
...
2020).
This thesis therefore assumes that the most advanced reporters in a sector therefore mark its practical
...
\midrule
Thai Union  & Thailand & Wild-capture tuna & GRI, in accordance & 53 & 76\% \\
Bolton      & Italy    & Wild-capture tuna & ESRS            & 45 & 64\% \\
Nomad Foods & UK       & Multi-species     & GRI, with reference & 42 & 60\% \\
Profand     & Spain    & Wild + aquaculture & GRI, with reference & 39 & 56\% \\
...
(Mowi, Bolton, Espersen); the other three use the GRI Standards, and not to the
same degree. Thai Union reports \emph{in accordance with} the GRI Standards,
the fuller of the two GRI application levels, while Nomad Foods and Profand
report \emph{with reference to} them, citing selected disclosures without
claiming full application. A bridge was therefore built to score all six on one
ruler. Each GRI disclosure was mapped to the ESRS topical DR(s) it addresses,
drawing on the published GRI-ESRS interoperability mapping (GRI \& EFRAG, 2024),
and adjudicated case by case where the correspondence is only partial. The
