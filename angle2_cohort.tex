...
from primary documents, what that practice is - and therefore the practical
\emph{upper bound} of sector
...
Figure~\ref{fig:methodology-overview}. %LI: the linking is not correct!

\textbf{Scope note: this is a coverage analysis, not a judgement of reporting
quality.} What follows measures whether a topic is disclosed at all, coded
from each firm's own published sustainability report or ESRS-index appendix
- note that it is not an assessment of whether any firm's sustainability performance,
policy substance, or reporting quality is good or bad. A firm scoring lower
on breadth may report less on a given topic for many reasons unrelated to
performance (a different production system, a different framework, a
different reporting year), and a firm scoring higher is not thereby judged
to perform better on the underlying sustainability issue itself. All figures
in this chapter describe \emph{public disclosure as written}, using only
information each firm has itself chosen to publish; no non-public data,
site visits or firm-specific verification were used, and no ranking or
evaluative claim about the six companies is intended or should be inferred
from the coverage numbers below. %LI: maybe a bit long of a disclaimer? shorten!
...
(Baumgartner \& Ebner, 2010), %LI: not sure this claim comes from this paper!
and the completeness and quality of a firm's
reporting can be read as a position on that maturity gradient (C\"oster et al.,
2020). %LI: not an existing source!
% CL: flagging for point 4 (unsourced claims) -- correction to your own %LI
% flags above: I checked references.tex directly. Both citations DO exist
% there with real DOIs -- Baumgartner & Ebner (2010, Sustainable Development
% 18(2)) and C\"oster, Dahlin \& Isaksson (2020, Sustainability 12(24), 10393,
% "A Measurement Maturity Grid for Evaluation of Sustainability Reports" --
% title is a near-exact match for this claim). So "not an existing source"
% looks like an outdated worry -- worth a quick look at the actual papers to
% confirm they say what's claimed here, but they're not fabricated/missing.
% Also candidate cut for point 2 (repetition): this same Baumgartner & Ebner
% "reactive to systematic" claim is restated again near the end of this
% chapter (Section~7.5-ish, "the maturity literature casts the move from
% reactive to systematic reporting as the hard step") -- once fixed/sourced
% here, that second instance could just cross-reference this one.
...
seafood processing - to disclose given real data systems and resources. %LI: should we say this research assumes therefore that ...
...
(DiMaggio \& Powell, 1983). Note that this is an interpretive reading, and a competing one
...
reporter covers, not merely an average across firms. %LI: rewrite this, too abstract!
...
firm's position in the chain. %LI: what tf does this mean? is this necessary info or just noise?
...
sourcing models. The aim of the selection is not representativeness but reach -
...
FY2024 results). Four of the six --- Bolton, Mowi, Nomad Foods and Thai Union
--- also appear among Mordor Intelligence's top-ranked players in the European
...
requirements (E1--E5, S1--S4, G1); see the \nameref{cohort-method} section.}
\label{tab:cohort-r}
...
Nomad Foods & UK       & Multi-species     & GRI             & 42 & 60\% \\
Profand     & Spain    & Wild + aquaculture & GRI            & 39 & 56\% \\
...
resources it commands. % LI: this is the caveat you flagged re: the
% capacity/reporting claim -- added to head off the size-drives-coverage
% reading before the table can be seen that way.
...
end-users; G1 Business conduct.} resolving into a defined,
...
published GRI-ESRS interoperability mapping (GRI \& EFRAG, 2024) %LI: link to existing document? I can provide if you don't have!
and adjudicated case by
...
(Chapter~4). This is a corroborating step rather
...
one firm (the remaining eight, reported by none, are examined in the
\nameref{cohort-shape} section),
26 by at least five of the six, and 13 by all six. This funnel is the
...
taken up in the cross-framework mapping and the worked case that follow. %LI: good sidenote but right positioning? maybe shorter - but keep simple language? 
...
screened set. %LI: this is not clear - screened set = all possible DRs you can report?
...
is deepest on own-workforce disclosure (S1, 16 of 17) and affected communities (S3, 5 of 5), Espersen is rather thin on
...
from firm to firm. %LI: ensure diplomatic language - they will read this equally!
...
the \emph{most} thoroughly covered pillar per firm on average (mean 72\% of
its 6 DRs, range 50--83\%, against a mean 52\% for Environmental and 62\% for
...
practices (G1-6). The sector's clearest collective gap is therefore
...
across the cohort. This gap should be read with care: the
...
reporting each DR, among the 62 DRs reported by at least one firm.
...
\subsection{Robustness across sourcing models and reporting
...
faces (Kittinger et al., 2017) - yet only one requirement is reported uniquely by it, so the consensus
...
\caption{Cohort-size invariance: the near-universal set (26 DRs) is
...
DRs, Profand from 40 to 39, and Espersen from 36 to 34 - driven by a handful of
...
review described in Chapter~4. And the analysis is a
