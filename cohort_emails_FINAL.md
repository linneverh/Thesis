# Cohort review emails — ready to send

**Attach to every version (4 files, 3.5 MB total):**

1. `Verhoeven_EPFL_thesis_draft.pdf` — the full draft
2. `ESRS_Coverage_Results.xlsx` — the results matrix (opens on a README sheet explaining all 11 tabs)
...
**1. The tool link is not ready yet.** The live site at `seafood-sme-priority-navigator.netlify.app` is still showing the old version — the `.txt` download, the old verbose copy, and the MSC/ASC error. I could not push the update from here (the sandbox blocks the deploy upload). To fix it in about a minute:

- Go to app.netlify.com → the `seafood-sme-priority-navigator` site → **Deploys**
- Rename `Seafood_SME_Priority_Navigator_v2.html` to `index.html`
- Drag it onto the deploy drop zone
- Reload the page and confirm the download button says `.xlsx`

Only paste the link into the emails after that check passes.

**2. Correction on the Excel files — you were right.** I said all nine workbooks open on a README sheet. Four of them don't:
