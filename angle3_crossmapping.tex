...
steps rather than one. Step~1 (Chapter~5) fixed the \emph{columns}: the
instruments that make up the disclosure ecosystem and how binding each is.
Step~2 (Chapter~6) fixed the \emph{rows}: the disclosure topics the sector's
leading firms actually report, narrowed to a cohort-evidenced universe.
Step~3, the \emph{overlap}, crosses the two, scoring, topic by topic and
instrument by instrument, \emph{who demands what}, and reading the resulting
matrix for the question the whole thesis exists to answer: given a
fragmented wall of overlapping demands, which topics, if built first,
satisfy the most of them. Step~4, \emph{building the tool}, turns that
finding into a decision a firm can act on: a short, ranked list of the
disclosure topics it should build first, and a diagnostic that tailors that
list to its own situation. Together these two steps produce the thesis's
central practical contribution and answer its third research question. A
representative excerpt of the underlying coverage matrix
(Figure~\ref{fig:coverage-matrix-screenshot}) is shown below so the scale
and shape of the raw scoring are visible before the chapter turns to how it
is built and scored.
...
revealed to be expected (26 requirement-level rows). \emph{(ii)} Two topics - biodiversity (E4) and consumer health and food safety (S4) - clear that same
...
not concern it.
The funnel is therefore the 70 topical ESRS disclosure
...
classification - scored, baseline, context or excluded, with the reason for each - is provided in \nameref{app:scope}.
...
indicator, and the identifier(s) of the ESRS datapoint(s) the demand maps to - the audit trail that protects the coding against challenge; the complete coded
...
The \emph{differences} are structured rather than arbitrary. Four further topics - water (E3-4), waste (E5-5), occupational health and safety (S1-14) and
...
profile, it returns a ranked Stage-1 build-first set - here eight themes - and states plainly how much of the demand environment building them
...
firms (member checking; Lincoln \& Guba, 1985). The \emph{explanatory} claim - that capabilities are shared across requirements, so that investment compelled by a
