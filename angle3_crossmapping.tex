...
% notes to self, same convention as elsewhere.
% =====================================================================
...
--- is provided in \nameref{app:scope}.
% numbers deliberately left in the source comments, not asserted in prose here --
% the 26+2+4 = 32 rows and the 49 -> 30 scored -> 31 columns are the current
% reconciled figures from Chapters 5-6; state them here once confirmed final.
...
% TWO follow-ups from the last review pass. (1) The EUDR cell under feed \&
% marine-ingredients sourcing is currently tagged direct in
...
% the feed supplier, so it should read indirect. Doesn't change the score
% (locus is descriptive only, per above) so it's a text/tagging fix, not a
% renumbering -- corrected the description here but haven't touched the
% workbook, since its Obligation tag is set once per instrument column
% rather than per cell, and retagging would also touch the two other rows
% EUDR scores on (biodiversity, traceability), not reviewed yet for the
% same question. Five-minute check in Excel when there's a moment.
% (2) Keeping the sector-material row count at four: ASC's
% conversion-free-sourcing criterion follows the same
% certification-precondition logic used above for feed and animal welfare,
% so it's noted as a scope limitation in Section~8.5 instead of added as a
% fifth row.
...
% worth being precise -- the crosswalk operates at disclosure-requirement
% level, not official-datapoint level (no ESRS datapoint is itself tagged
% "start-here"; a VSME datapoint mapped onto a theme's DR shows a lighter,
% self-assessable version of that DR exists). Checked against the
% nine-topic Stage-1 set: five of the seven ESRS-anchored topics have
% solid VSME coverage (energy, GHG emissions, water, waste, health and
% safety); biodiversity only partial; value-chain workers (S2-1) none at
% all -- confirms at datapoint resolution the gap already flagged in
% \nameref{app:esrs-mapping}. The four seafood-specific themes with no ESRS
% requirement (wild-stock, feed, animal health/antimicrobial use,
% traceability) are sourced from GRI~13 instead, correctly no VSME entry
% either. Two themes illustrated here with the enabling method that
% produces their data (GHG Protocol for emissions, GDST for traceability)
% -- not yet built into a systematic table covering every Stage-1 topic,
% flagging as outstanding rather than overclaiming coverage.
Each \emph{topic} (not
