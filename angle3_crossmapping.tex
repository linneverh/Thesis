...
This chapter covers two steps rather than one. Step~1 (Chapter~5) fixed the
\emph{columns}: the instruments that make up the disclosure ecosystem and how
binding each is. Step~2 (Chapter~6) fixed the \emph{rows}: the disclosure topics
...
the cohort frontier (Chapter~6) and are built up in three steps:
...
practice has revealed to be expected (26 requirement-level rows).

\item \textbf{Two topics clear the same bar one level up.} Biodiversity (E4) and
consumer health and food safety (S4) are each reported by at least five firms when
a firm counts as covering the topic if it reports \emph{any} of the topic's
requirements. No individual E4 or S4 requirement reaches the five-of-six cut on its
own. They therefore enter as topic-level rows: the same consensus rule at coarser
granularity, not an expert-judgement exception. With the requirement-level set they
form the cohort-evidenced core (28 rows).
...
\emph{Structural gap} (no ESRS disclosure requirement exists at all). The four
split 2+2. Animal health and welfare and supply-chain traceability are
Structural-gap topics with no ESRS home whatsoever. Wild-stock status and feed and
marine-ingredient sourcing are production-specific disclosures nested inside the
Biodiversity topic (GRI~13.3), which is itself Covered and already in the universe:
the topic is covered, but these two disclosures within it are not. The full
...
outside the demand sums (\nameref{cm-matrix}). Eight of the nine Stage-1 topics
...
serves, taken from the matrix (\nameref{cm-staging}). This makes the reuse
...
\nameref{cm-staging} logic takes for an individual firm. The mixed profile
