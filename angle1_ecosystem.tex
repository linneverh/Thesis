...
inclusion against two gates (below). Third, the resulting classification --- each
...
% CL: spell out "SFE" in full on first use. And confirm this three-step description
% CL: matches what you actually did (cohort-report screen + regulatory scan + expert review).
...
tier, domain, source and locus, each instrument therefore also carries a status
...
institutional isomorphism (DiMaggio \& Powell, 1983). The four tiers make that
...
packaging law, occupational-safety law. This is coercive obligation in its purest form. %LI: source coercive obligation?
...
the mimetic-normative middle, where firms adopt what peers and buyers expect. %LI: source mimetic-normative obligation? or where do we explain?
...
A reactive firm operates only within Tiers~1 and~2; extending engagement into
Tiers~3 and~4 is precisely what the reactive-to-proactive shift (Section~3.2)
consists of --- which is why the necessity axis, and not merely a list of
instruments, is the structuring device.

%LI: is this actually something we say in phase 3 as well - i think in phase 4 we just say if metnioned by x frameworks it's build first - maybe check if this aligns actually?
...
EU Deforestation Regulation binds the processor directly). Where it does not, the
...
obligated, certified or reporting entity (the EU Deforestation Regulation) %LI: use different example because EU DEFORESTATION not specific to seafood so weird to have as example;
